...
export const NATIVE_GLOBAL_EVENTS = [
    'animationcancel',
    'animationend',
    'animationiteration',
    'animationstart',
...
export const omitEventCallbacks = (customEvents, props) => {
...
export const toPascalCase = (kebabText) => kebabText.replace(/(^\w|-\w)/g, clearAndUpper);
const toCallbackName = (name) => `on${toPascalCase(name)}`;
export const useEventListeners = (ref, customEvents, props) => {
...
export const gcdsAttributeGenerator = (tagName, props) => {
    if (props['class-name']) {
